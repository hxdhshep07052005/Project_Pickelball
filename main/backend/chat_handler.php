...
$msgLower = function_exists('mb_strtolower')
    ? mb_strtolower($userMessage, 'UTF-8')
    : strtolower($userMessage);

$pickleballKeywords = [
    'pickleball', 'paddle', 'serve', 'dink', 'volley', 'smash', 'forehand', 'backhand', 'drive',
    'swing', 'stance', 'footwork', 'follow through', 'follow-through', 'rotation', 'balance',
    'analysis', 'video', 'technique', 'training', 'practice', 'drill', 'warm-up', 'warmup', 'coach',
    'pickleball', 'vợt', 'giao bóng', 'đỡ bóng', 'đập bóng', 'thuận tay', 'trái tay',
    'kỹ thuật', 'phân tích', 'video', 'luyện tập', 'bài tập', 'huấn luyện', 'cú đánh'
];

$analysisRefPhrases = [
    'this technique', 'this swing', 'this analysis', 'my analysis', 'my video',
    'kỹ thuật này', 'phân tích này', 'video của tôi', 'cú đánh này'
];

$isPickleballQuestion = false;
foreach ($pickleballKeywords as $kw) {
    if (strpos($msgLower, $kw) !== false) {
        $isPickleballQuestion = true;
        break;
    }
}
if (!$isPickleballQuestion) {
    foreach ($analysisRefPhrases as $phrase) {
...
if (!$isPickleballQuestion) {
    ...
    echo json_encode([
        'success' => true,
        'response' => 'I can only answer questions related to pickleball training and your video analysis. Please ask a pickleball-related question.',
...
try {
    ...
    try {
        ...
        $systemPrompt .= "Answer ONLY the user's current question. ";
        $systemPrompt .= "Do NOT invent extra questions, sections, or assumptions. ";
        $systemPrompt .= "Do NOT use rigid templates unless the user asks for a template. ";
        $systemPrompt .= "Use the analysis context only as supporting evidence for your answer. ";
        $systemPrompt .= "If the question is unrelated to the analysis, answer it naturally and briefly. ";
        $systemPrompt .= "If information is insufficient, ask one short clarifying question. ";
        $systemPrompt .= "Respond in the same language as the user's question. ";
        $systemPrompt .= "Keep the answer practical, specific, and concise.";
        ...
        $messages[] = [
            'role' => 'assistant',
            'content' => "Analysis context (for reference only):\n" . $contextMessage
        ];
