...
from prompt import backhand_prompt, backhand_evidence_prompt
...
def _parse_keyword_fallback(answer: str) -> dict:
    ...
    if any(s in text for s in ["hands together", "both hands together", "connected", "both hands on paddle"]):
...
def _retry_unknown_fields(
    ...
    for field in _SCHEMA_KEYS:
        if updated[field] != "unknown":
            continue
        prompt = "Question: Analyze this pickleball two-handed-backhand-drive keyframe. " + _FIELD_PROMPTS[field] + " Answer:"
...
def analyze_backhand_frame(image_path: str, vision_llm_client: dict) -> dict:
    ...
    evidence_text = _generate_answer(
        image=image_for_model,
        prompt=backhand_evidence_prompt(),
        processor=processor,
        model=model,
        device=device,
        bad_words_ids=bad_words_ids,
