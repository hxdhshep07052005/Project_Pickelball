...
async def analyze_video(request: AnalyzeRequest):
    ...
    try:
        ...
        llm_analyses = {}
        llm_status_message = "Vision LLM analysis completed"
        if VISION_LLM_AVAILABLE:
            try:
                ...
                if vision_llm_client is None:
                    llm_status_message = "Vision LLM is still loading in background. This run used pose-only analysis; please retry shortly for BLIP-2 analysis."
                ...
                llm_analyses = {}
                llm_status_message = f"Vision LLM failed during analysis: {str(e)}"
        else:
            print("Vision LLM not available, skipping Vision LLM analysis")
            llm_status_message = "Vision LLM module not available. This run used pose-only analysis."
        ...
        return JSONResponse({
            ...
            "vision_llm_status": llm_status_message,
