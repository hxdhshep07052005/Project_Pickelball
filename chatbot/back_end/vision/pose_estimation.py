...
try:
    ...
    try:
        ...
        # MediaPipe not available at all
        print(f"Warning: MediaPipe not available: {e}", file=__import__('sys').stderr)
        print("Falling back to extraction without pose detection", file=__import__('sys').stderr)
        USE_NEW_API = None
        mp_pose = None
        mp = None
        MEDIAPIPE_AVAILABLE = False
except ImportError as e:
    # MediaPipe not installed at all
    print(f"Warning: MediaPipe not available: {e}", file=__import__('sys').stderr)
    print("Falling back to extraction without pose detection", file=__import__('sys').stderr)
    USE_NEW_API = None
    mp_pose = None
...
def extract_pose_from_frame(image_path: str, pose) -> dict | None:
    ...
    if USE_NEW_API:
        # MediaPipe 0.10+ API
        # Create image from numpy array (more reliable than file path)
        from mediapipe import ImageFormat
...
def process_frame_folder(
    ...
    if not MEDIAPIPE_AVAILABLE or USE_NEW_API is None:
        raise ImportError("MediaPipe is not available. Please install mediapipe package: pip install mediapipe")
    ...
    if USE_NEW_API:
        ...
        if not os.path.exists(model_path):
            try:
                ...
                print(f"Downloading pose model to {model_path}...", file=__import__('sys').stderr)
                urllib.request.urlretrieve(model_url, model_path)
                print("Model downloaded successfully.", file=__import__('sys').stderr)
