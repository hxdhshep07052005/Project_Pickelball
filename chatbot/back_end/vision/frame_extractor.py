...
def extract_frames(
    ...
    if use_pose_detection:
        if not MEDIAPIPE_AVAILABLE or USE_NEW_API is None:
            ...
            try:
                if USE_NEW_API:
                    # MediaPipe 0.10+ API - for frame extraction, we can skip pose detection
                    # as it's mainly used for scoring, not critical
                    print("Warning: MediaPipe 0.10+ detected. Pose detection in frame extraction may not work optimally.")
                    print("Continuing without pose detection in frame extraction.")
                    use_pose_detection = False
                    pose = None
                else:
