...
def load_vision_llm_analyses(llm_analyses_path: str) -> Dict[str, dict]:
    """
    Load Vision LLM analyses from JSON file.
    
    Args:
        llm_analyses_path: Path to Vision LLM analyses JSON file
    
    Returns:
        Dictionary mapping frame filename to LLM analysis data
    """
...
def combine_pose_and_llm(
    ...
    Args:
        pose_frames: List of pose data from MediaPipe (from load_poses)
        llm_analyses: Dictionary mapping frame filename to LLM analysis
    
    Returns:
        List of combined data dictionaries with both pose and LLM information
    """
    combined_frames = []
...
def save_combined_data(combined_frames: List[dict], output_path: str):
    """
    Save combined pose + Vision LLM data to JSON file.
    
    Args:
        combined_frames: List of combined frame data
