...
import torch
from prompt import backhand_prompt


def analyze_backhand_frame(image_path: str, vision_llm_client: dict) -> dict:
    """
    Analyze a single frame using Hugging Face Vision-Language model for backhand.
...
def _parse_text_to_structured_format(text: str) -> dict:
    """
    Parse unstructured text description into structured format for backhand.
    ...
    if any(term in text_lower for term in ["hands together", "hands connected", "both hands together", "hands close"]):
        hand_position = "together"
    elif any(term in text_lower for term in ["hands separated", "hands apart", "hands disconnected", "hands far"]):
        hand_position = "separated"
    
    arm_structure = "unknown"
    if any(term in text_lower for term in ["arms extended", "both arms extended", "arms straight", "good arm structure"]):
        arm_structure = "extended"
    elif any(term in text_lower for term in ["arms bent", "elbows bent", "collapsed", "arms collapsed", "elbows collapsed"]):
        arm_structure = "collapsed"
    elif any(term in text_lower for term in ["partially extended", "somewhat extended", "moderate extension"]):
        arm_structure = "partial"
    ...
    if any(term in text_lower for term in ["good rotation", "proper rotation", "adequate rotation", "sufficient rotation", "good torso rotation"]):
        body_rotation = "good"
    elif any(term in text_lower for term in ["insufficient rotation", "lack of rotation", "minimal rotation", "not enough rotation", "limited rotation"]):
...
def analyze_frames_batch(frame_dir: str, vision_llm_client: dict, frame_files: List[str] = None) -> List[Dict]:
    """
    Analyze multiple frames in a directory using Hugging Face Vision-Language model for backhand.
    ...
    for frame_file in frame_files:
        ...
        try:
            ...
            analysis = analyze_backhand_frame(frame_path, vision_llm_client)
