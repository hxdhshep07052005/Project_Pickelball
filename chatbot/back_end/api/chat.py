...
async def chat(request: ChatRequest):
    ...
    combined_data_file = os.path.join(BASE_DATA_DIR, "combined", f"{session_id}_combined.json")
    ...
    try:
        ...
        # Check if combined data exists (optional - for Vision LLM insights)
        combined_data_path = None
        if os.path.exists(combined_data_file):
            combined_data_path = combined_data_file
            print(f"Found combined data file: {combined_data_file}")
        else:
            print(f"Combined data file not found (optional): {combined_data_file}")
        
        # Build LLM messages (includes Vision LLM insights if available)
